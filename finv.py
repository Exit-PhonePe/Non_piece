...
# ==========================================
    ...
    if st.session_state.get('final_master_df') is not None:
        ...
        abs_data = curr_df[abs_mask]
        
        if len(abs_data) > 0 and st.session_state.absconding_decision == 'pending':
            st.markdown(f"<div class='alert-popup'><h2>⚠️ {len(abs_data)} Separation Cases Detected!</h2></div>", unsafe_allow_html=True)
            c1, c2 = st.columns(2)
            with c1: 
            ...
            edited = st.data_editor(abs_data, use_container_width=True, key="abs_editor")
            if st.button("✔️ Save Edits & Finalize", use_container_width=True):
                if 'Employee ID' in edited.columns:
                    edited_cols = edited.copy()
                else:
                    edited_cols = edited.reset_index()

                curr_df.set_index('Employee ID', inplace=False)
                edited_cols.set_index('Employee ID', inplace=True)
                
                final_merged_df = curr_df.set_index('Employee ID')
                final_merged_df.update(edited_cols)
                
                st.session_state.final_master_df = final_merged_df
                st.session_state.absconding_decision = 'done'; st.rerun()
        else:
